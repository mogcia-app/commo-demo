...
export function TemplateReserveForm({ templateType }: { templateType: TemplateType }) {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push(`/reserve/complete?industry=${config.industryType}&template=${config.templateType}`);
    } catch (cause) {
  ...
  return (
    ...
    >
      <TemplateHero config={config} />
      <TemplateProfile config={config} profile={profile} liffState={liffState} />
...
function TemplateHero({ config }: { config: ReservationTemplateConfig }) {
  return (
    ...
    <section className="mb-4 rounded-md border border-purple-100 bg-white/90 p-4 shadow-sm">
      ...
      <div className="mt-3 flex items-center gap-3">
        {profile?.pictureUrl ? (
          ...
          <div className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-sm font-bold" style={{ color: config.accent }}>
            L
          </div>
        )}
